...
$(function(){
	...
	function enableReordering()
	{
		...
		$('.tabs .selected').each(function()
		{
			...
			if ( section == 0 )
			{
				$(this).append('<a class="btn delete_section disabled" href="#" title="You can\'t delete the first section" onclick="return false;"><i class="icon-ban-circle"></i></a>');
			}
			else
			{
		...
		$('.tabs > ul li:not(:first-child):not(#tab_add)').addClass('movey');

		$('.tabs > ul').sortable({
			items: 'li:not(:first-child):not(#tab_add)', //skip section 0, and the add tab button
			scroll: false ,
